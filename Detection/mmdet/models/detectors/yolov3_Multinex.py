...
class Multinex_YOLOV3(SingleStageDetector):
    ...
    def __init__(self,
                 ...
                 pretrained=None,
                 init_cfg=None):
        super(Multinex_YOLOV3, self).__init__(backbone, neck, bbox_head, train_cfg,
    ...
    def mseLoss(self, x_det, img):
        total_loss = nn.MSELoss()(x_det, img)
        return total_loss * self.loss_factor

    def extract_feat(self, img):
        """Directly extract features from the backbone+neck."""
        x_det, delta = self.pre_encoder(img)  # ERE

        # loss_decom = self.mseLoss(x_det, img)
        loss_decom = (delta**2).mean() * self.loss_factor
        
        x = self.backbone(x_det)
